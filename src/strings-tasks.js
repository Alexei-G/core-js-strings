...
function getStringLength(value) {
  const placeCount = value ? value.length : 0;
  return placeCount;
}
...
function isPalindrome(str) {
  ...
  const revStr = normStr.split('').reverse().join('');
  return normStr === revStr;
}
...
function extractNameFromTemplate(value) {
  const templ = value;
  const startPos = templ.lastIndexOf(',') + 2;
  const endPos = templ.lastIndexOf('!');
  const sliced = templ.slice(startPos, endPos);
  return sliced;
}
